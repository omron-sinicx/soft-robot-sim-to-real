...
class PPOStorage:
    def __init__(self, num_inputs, num_outputs, max_size=64000, horizon=20):
        self.states = torch.zeros(max_size, horizon, num_inputs).to(device)
...
class RL(object):
    def __init__(self, env, hidden_layer=[64, 64]):
        ...
        # self.horizon = int(args.param1)
        # self.kernel_size = int(args.param2)
        self.horizon = 20
        self.kernel_size = 5

        self.Net = ActorCriticNetTCN
        self.model = self.Net(self.num_inputs, self.num_outputs, self.hidden_layer, kernel_size=self.kernel_size)
        ...
        self.gpu_model = self.Net(self.num_inputs, self.num_outputs, self.hidden_layer, kernel_size=self.kernel_size)
        self.gpu_model.to(device)
        self.model_old = self.Net(self.num_inputs, self.num_outputs, self.hidden_layer, kernel_size=self.kernel_size).to(device)
    ...
    def collect_samples_vec(self, num_samples, start_state=None, noise=-2.5, env_index=0, random_seed=1):
        ...
        state = start_state
        # TODO: handle edge case that start state is a termination state
        if self.history_storage.get_size() < self.history_storage_size:
            self.history_storage.push(state, torch.zeros(1).to(device), False)
        else:
            self.history_storage.clear()
            truncated = True
            self.history_storage.push(state, torch.zeros(1).to(device), True)
        total_reward1 = 0
        total_reward2 = 0
        ...
        while samples < num_samples:
            with torch.no_grad():
                history_current = torch.vstack((self.history_storage.get_recent_sensor_history(), state))[None, :]

                input = history_current

                action, mean_action = self.gpu_model.sample_actions(input)
            ...
            if self.history_storage.get_size() < self.history_storage_size:
                self.history_storage.push(state, torch.zeros(1).to(device), terminated or truncated)
            else:
                self.history_storage.clear()
                truncated = True
        ...
        R = self.gpu_model.get_value(input)
        # import ipdb; ipdb.set_trace()
        while counter >= 0:
    ...
    def fresh_update(self):
        fresh_model = self.Net(self.num_inputs, self.num_outputs, self.hidden_layer, kernel_size=self.kernel_size).to(device)
        fresh_model.train()
    ...
    def collect_samples_multithread(self):
        ...
        total_thread = 0
        max_samples = 1000
        self.history_storage_size = 10000    
        self.storage = PPOStorage(self.num_inputs, self.num_outputs, max_size=max_samples, horizon=self.horizon+1)
        self.history_storage = Storage(sensor_dim=self.num_inputs, privileged_dim=1, horizon=self.horizon, max_size=self.history_storage_size)
        ...
        for iterations in range(100000):
            ...
            if (iterations) % 1000 == 0:
                ...
                #     self.save_dir + "{}_reward_stats.npy".format(self.experiment_name)
                # )
...
if __name__ == "__main__":
    ...
    ppo.save_dir = "learning_progress_tcn/{}/".format(args.name)
